                    ...
                    <div class="card-body">
                        <div class="invoice-header">
                            ...
                            <div class="billed-from">
                                <h6>.منشئ الفاتورة : {{$details->user->name}}</h6>
                                <p>الأماكن الفروع : 6 ش جيوشي جيزة , 17 ش عمر بن خطاب مصر الجديدة <br>
                                    رقم الفرع الرئيسي: 324 445-4544<br>
                                    ايميل : {{$details->user->email}}</p>
                            </div><!-- billed-from -->
                        </div><!-- invoice-header -->
                        <div class="row mg-t-20">
                            <div class="col-md">
                                ...
                                <div class="billed-to">
                                    <h6>{{$invoices->section->title}}</h6>
                                    <p>{{$invoices->section->body}}<br>
                                        <br>
                            ...
                            <div class="col-md">
                                ...
                                <p class="invoice-info-row"><span>تاريخ الاصدار</span>
                                    <span>{{ $invoices->invoice_date->format('Y-m-d') }}</span></p>
                                <p class="invoice-info-row"><span>تاريخ الاستحقاق</span>
                                    <span>{{ $invoices->due_date->format('Y-m-d') }}</span></p>
                                <p class="invoice-info-row"><span>القسم</span>
                                    <span>{{ $invoices->section->title }}</span></p>
                            </div>
                        </div>
                        <div class="table-responsive mg-t-40">
                            <table class="table table-invoice border text-md-nowrap mb-0">
                                ...
                                <tr>
                                    <td class="tx-right">المبلغ الكلي شامل الضريبة</td>
                                    <td class="tx-right tx-danger" colspan="2"> {{ number_format($invoices->total + $invoices->amount_collection, 2) }}</td>

                                </tr>
                                <tr>
                                    <td class="tx-right tx-uppercase tx-bold tx-inverse">المبلغ المدفوع</td>
                                    <td class="tx-right" colspan="2">
                                        <h4 class="tx-primary tx-bold">{{ number_format($invoices->total + $invoices->amount_collection - $details->remaining_payment , 2) }}</h4>
                                    </td>
                                </tr>
                        ...
                        <a id="invoice_back" href="{{url('invoices/')}}" class="btn btn-success float-left mt-3 mr-2">
                            الصفحة الرئسية
                        </a>
