...
class InvoicesController extends Controller
{
    ...
    {
        $invoices = Invoices::query()->latest()->with(['section', 'product', 'status:id,name'])->get();
        $statuses = Status::all();
    ...
    public function create()
    {
        try {
            ...
            $invoice_number = $latestInvoice && is_numeric($latestInvoice) ? $latestInvoice + 1 : rand(1, 1000);

            while (Invoices::withTrashed()->where('invoice_number', $invoice_number  )->exists()) {
                $invoice_number = rand(1, 1000);
            }
